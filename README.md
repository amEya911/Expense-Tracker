...
### 1. Stored-Value Transit (PRESTO) and Zero-Double-Counting Financial Engine
A fundamental challenge in personal expense tracking is handling prepaid/stored-value transit cards (such as PRESTO, Octopus, Oyster) without distorting cash outflows:
...
- **Student Financial Insights Engine:**
  ...
  - *Daily Burn Rate and Runway:* Calculates real-time daily burn rate and projected month-end totals.
  - *Largest Single Outflow Tracker.*

---

### 3. Multi-Channel Payment and Currency Support
...
graph TD
    subgraph UI_Layer ["SwiftUI Views"]
        DashboardView["DashboardView (Home & PRESTO Widget)"]
        TransactionView["TransactionListView (Full Ledger & Search)"]
        AddExpenseView["AddExpenseView (Dynamic Form & Haptics)"]
        BudgetView["BudgetOverviewView (Charts & Targets)"]
        SettingsView["CategoryManagementView (Custom CRUD)"]
    end

    subgraph ViewModel_Layer ["Observable ViewModels"]
    ...
    subgraph Service_Layer ["Services"]
        ...
        FirestoreService["FirestoreService (Firestore CRUD & Metrics)"]
    end

    subgraph Backend_Layer ["Cloud Infrastructure"]
    ...
    DVM --> FirestoreService
    TVM --> FirestoreService
    EVM --> FirestoreService
    BVM --> FirestoreService
    AVM --> FirestoreService
...
│   ├── ExpenseCategory.swift      # Category model with SF Symbols and hex colors
│   ├── Budget.swift               # Monthly spending limits and category allocations
│   ├── UserProfile.swift          # User metadata and preferences
│   └── AnyCodableValue.swift      # Polymorphic custom field encoder
├── ViewModels/
│   ├── DashboardViewModel.swift   # Home screen orchestration and PRESTO metrics
│   ├── ExpenseViewModel.swift     # Form validation, balance checks and haptics
│   ├── TransactionViewModel.swift # Search, filtering and swipe-to-delete
│   ├── BudgetViewModel.swift      # Category budget allocations and progress
│   ├── AnalyticsViewModel.swift   # Swift Charts time-series and insight engines
│   ├── AuthViewModel.swift        # Authentication state flow
│   └── SettingsViewModel.swift    # Category management and data controls
├── Views/
│   ├── Dashboard/
│   │   ├── DashboardView.swift    # Hero spend card, metallic PRESTO widget and pacing
│   │   └── PrestoTopUpSheet.swift # 1-tap rapid top-up modal with preset chips
│   ├── Expenses/
│   │   └── AddExpenseView.swift   # Adaptive keypad and mandatory field validation
│   ├── Transactions/
│   │   └── TransactionListView.swift # Date-grouped transaction ledger and search
│   ├── Budgets/
│   │   └── BudgetOverviewView.swift  # Swift Charts analytics and budget target hub
│   ├── Settings/
│   │   ├── SettingsView.swift
│   │   └── CategoryManagementView.swift # Full Category CRUD and auto-seeder
│   └── Components/
│       ├── ProgressRing.swift     # Smooth animated circular progress ring
│       └── PrivacyOverlayView.swift
├── Services/
│   ├── AuthService.swift          # Anonymous and Email Firebase authentication
│   └── FirestoreService.swift     # Isolated subcollections and stored-value aggregation
├── Utilities/
│   ├── Constants.swift            # 12 default student categories and suggestions
│   ├── CurrencyFormatter.swift    # Locale-safe currency and decimal formatting
│   └── DateHelpers.swift          # Month-year grouping and calendar helpers
├── firestore.rules                # Strict user-isolated security rules
└── Expense_TrackerApp.swift       # App lifecycle and AppDelegate Firebase initialization
```
